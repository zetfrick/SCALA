...
class GUI(studentService: StudentService[IO], repo: Repository[IO]) extends JFXApp3 {
  ...
  private def addStudentTab(): Tab = {
    ...
    val addButton = new Button("Добавить студента") {
      onAction = _ => {
        clearError()
        try {
          val id = idField.text.value.toInt
          val name = nameField.text.value
          val course = courseField.text.value.toInt
          val group = groupField.text.value
          ...
          case _: NumberFormatException => showError("ID и курс должны быть числами")
          case ex: Exception => showError(s"Ошибка: ${ex.getMessage}")
        }
      }
    }
    ...
    val viewButton = new Button("Просмотреть студента") {
      onAction = _ => {
        ...
        try {
            ...
            _ <- IO {
              studentOpt match {
                case Some((student, grades)) =>
                  ...
                  sb.append(s"Курс: ${student.course}, Группа: ${student.groupName}\n\n")
                  sb.append("Оценки:\n")
                  grades.groupBy(_._1).foreach { case (subject, grades) =>
                    sb.append(s"$subject: ${grades.map(_._2).mkString(" ")}\n")
                  }
          ...
          case _: NumberFormatException => showError("ID должен быть числом")
          case ex: Exception => showError(s"Ошибка: ${ex.getMessage}")
        }
      }
    }
      ...
      onAction = _ => {
        ...
        try {
          ...
          val program = for {
            ...
            _ <- IO {
              idField.text = ""
              showError("Студент и его оценки удалены")
            }
          } yield ()
  ...
  private def addGradeTab(): Tab = {
    ...
    val addButton = new Button("Добавить оценку") {
      onAction = _ => {
        clearError()
        try {
          val studentId = studentIdField.text.value.toInt
          val subject = subjectField.text.value
          val grade = gradeField.text.value.toInt
          ...
          case _: NumberFormatException => showError("ID студента и оценка должны быть числами")
          case ex: Exception => showError(s"Ошибка: ${ex.getMessage}")
        }
      }
    }
  ...
  private def viewStatsTab(): Tab = {
    ...
    val viewButton = new Button("Просмотреть статистику") {
      onAction = _ => {
        ...
        program.unsafeRunAsync {
          case Left(ex) => showError(s"Ошибка при получении статистики: ${ex.getMessage}")
          case Right(_) => ()
        }
      }
    }
  ...
  private def viewAllDataTab(): Tab = {
    ...
    val viewButton = new Button("Просмотреть все данные") {
      onAction = _ => {
            ...
            groupedByCourse.foreach { case (course, courseStudents) =>
              ...
              groupedByGroup.foreach { case (groupName, groupStudents) =>
                ...
                groupStudents.foreach { case (student, grades) =>
                  ...
                  if (grades.nonEmpty) {
                    sb.append("    Оценки:\n")
                    grades.groupBy(_.subjectName).foreach { case (subject, subjectGrades) =>
                      sb.append(s"      $subject: ${subjectGrades.map(_.value).mkString(" ")}\n")
                    }
                  } else {
        ...
        program.unsafeRunAsync {
          case Left(ex) => showError(s"Ошибка при получении данных: ${ex.getMessage}")
          case Right(_) => ()
        }
      }
    }
