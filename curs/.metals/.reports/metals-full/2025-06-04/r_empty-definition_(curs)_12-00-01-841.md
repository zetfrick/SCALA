error id: file:///D:/ИРИТ/SCALA/curs/src/main/scala/Main.scala:_empty_/`<import>`.
...
class ConsoleApi(studentService: StudentService[IO], repo: Repository[IO]) {
  ...
  private def readIntInput(prompt: String): IO[Int] =
    readInput(prompt).flatMap { input =>
      IO(input.toInt).handleErrorWith { _ =>
        IO.println("Ошибка: введите целое число") >> IO.raiseError(
          new IllegalArgumentException("Некорректный ввод числа")
        )
      }
    }
  ...
  private def addStudent(): IO[Unit] =
    (for {
      id <- readIntInput("ID студента: ").attempt
      _ <- id match {
        case Left(_) => IO.unit
        case Right(id) =>
          repo.getStudent(id).flatMap {
            ...
            case None =>
              for {
                ...
                course <- readIntInput("Курс: ").attempt
                _ <- course match {
                  case Right(course) =>
                    for {
                      groupName <- readInput("Группа: ")
                      _ <- studentService
                        .createStudent(id, name, course, groupName) >>
                        IO.println("Студент успешно добавлен")
                    } yield ()
                  case Left(_) =>
                    IO.println("Ошибка: курс должен быть числом")
                }
              } yield ()
          }
      }
  ...
  private def viewStudent(): IO[Unit] =
    (for {
      ...
      _ <- id match {
        case Right(id) =>
          studentService.getStudentDetails(id).flatMap {
            ...
            case None =>
              IO(println("Студент не найден"))
          }
        case Left(_) => IO.println("Ошибка: ID студента должен быть числом")
      }
    } yield ()).handleErrorWith(_ => IO.unit)

  private def deleteStudent(): IO[Unit] =
    (for {
      id <- readIntInput("ID студента для удаления: ").attempt
      _ <- id match {
        case Right(id) =>
          studentService.deleteStudentWithGrades(id) >>
            IO.println("Студент и его оценки удалены")
        case Left(_) => IO.println("Ошибка: ID студента должен быть числом")
      }
    } yield ()).handleErrorWith(_ => IO.unit)
  ...
  private def addSubject(): IO[Unit] =
    (for {
      ...
      _ <- repo.getSubject(name).flatMap {
        case Some(_) =>
          IO.println("Ошибка: предмет с таким названием уже существует")
        case None =>
          repo.addSubject(Subject(name)) >>
            IO.println("Предмет успешно добавлен")
      }
    } yield ()).handleErrorWith(_ => IO.unit)

  private def addGrade(): IO[Unit] =
    (for {
      studentId <- readIntInput("ID студента: ").attempt
      _ <- studentId match {
        case Right(studentId) =>
          repo.getStudent(studentId).flatMap {
            case None => IO.println("Ошибка: студент с таким ID не существует")
            case Some(_) =>
              for {
                subjectName <- readInput("Название предмета: ")
                _ <- repo.getSubject(subjectName).flatMap {
                  case None =>
                    IO.println(
                      "Ошибка: предмет с таким названием не существует"
                    )
                  case Some(_) =>
                    for {
                      value <- readIntInput("Оценка (1-5): ").attempt
                      _ <- value match {
                        case Right(value) if value >= 1 && value <= 5 =>
                          repo.addGrade(Grade(studentId, subjectName, value)) >>
                            IO.println("Оценка успешно добавлена")
                        case Right(_) =>
  ...
  private def viewAllData(): IO[Unit] =
    (for {
      ...
      _ <- IO {
        ...
        // Группируем сначала по курсу, затем по группе
        val groupedByCourse = allData
          .groupBy { case (student, _) =>
            student.course
          }
          .toList
          .sortBy(_._1)
  ...
  def run(): IO[Unit] = {
    val commands = Stream.iterate(0)(_ + 1).evalMap { _ =>
      for {
        ...
        choice <- readInput("")
        _ <- choice match {
          case "1" => addStudent().handleErrorWith(_ => IO.unit)
          case "2" => viewStudent().handleErrorWith(_ => IO.unit)
          case "3" => deleteStudent().handleErrorWith(_ => IO.unit)
          case "4" => addSubject().handleErrorWith(_ => IO.unit)
          case "5" => addGrade().handleErrorWith(_ => IO.unit)
          case "6" => viewStats().handleErrorWith(_ => IO.unit)
          case "7" => viewAllData().handleErrorWith(_ => IO.unit)
